...
#include <cmath>
#ifndef DUALNUMBERS_MATH_H_
#define DUALNUMBERS_MATH_H_
...
{
    template<class T>
    constexpr T operator()(T const& x) { return x * power<N-1>{}(x); }
};
...
{
    template<class T>
    constexpr T operator()(T const& x) { return T{1}; }
};
...
constexpr float exp_impl(float const& x, std::index_sequence<Is...>)
{
    ...
    static_cast<void>(unused{ 0, (ret += power<Is>{}(x) / static_cast<float>(factorial<Is>()), 0)... });
